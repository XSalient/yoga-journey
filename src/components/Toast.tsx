...
const ToastItem: React.FC<{ toast: ToastType; onClose: () => void }> = ({ toast, onClose }) => {
  const baseStyles = 'p-4 rounded-md shadow-lg flex items-center gap-3 animate-slide-in';
  const typeStyles = {
  ...
  return (
    <div className={`${baseStyles} ${typeStyles[toast.type]}`}>
      ...
      <button
        onClick={onClose}
        className="flex-shrink-0 text-opacity-70 hover:text-opacity-100 transition-colors"
